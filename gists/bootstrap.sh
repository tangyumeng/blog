...
rm -rf build.log
rm -rf ~/Library/Developer/Xcode/DerivedData/RxPlayground-* 2> /dev/null
rm -rf RxPlayground.xcworkspace/xcuserdata
rm -rf Libs
...
## Get RxSwift
RXSWIFT_CACHE="/tmp/playground/RxSwift/${RXSWIFT_VERSION}"
...
xcodebuild build -scheme RxSwift -workspace RxPlayground.xcworkspace -sdk iphonesimulator -destination "name=iPhone 8" &> build.log & BUILDPID=$!
loader $BUILDPID
